...
var program = require('commander'), // работа с командной строкой
    path = require('path'),
    findit = require('findit'), // поиск файлов
    fs = require('fs'),
    async = require('async'), // паралельность
    tools = require('./tools/all'), // утилиты
    IMGO = {
        CPUs:require('os').cpus().length * 2, // по два потока на процессор
        filesExt:new Array()
    },
    file = {},
    fileList = [],
...
function saveFile(item, callback) {
    setTimeout(function () {
        console.log('saveFile', item, 'size:', tools.getFileSize(item));
        callback();
    }, Math.random() * 2000);
}
